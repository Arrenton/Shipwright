...
extern "C" int GetLeveledNaviEnemyInfo(char* buffer, char* src, const int maxBufferSize, Actor* actor) {
    ...
    if (gSaveContext.language == LANGUAGE_FRA) {
        ...
        postfix = "";
        if (CVar_GetS32("gLeveledNaviLevel", 1)) {
            postfix += " \x05" "F" "Lv" + std::to_string(actor->level);
        }
        if (CVar_GetS32("gLeveledNaviMaxHP", 1) && actor->maximumHealth > 0) {
            postfix += " \x05" "A" "MaxHP " + std::to_string(actor->maximumHealth);
        }
    }
    //CustomMessageManager::Instance->FormatCustomMessage(postfix);
    std::string str;
