...
void Leveled_DrawTexI8(PlayState* play, void* texture, s16 textureWidth, s16 textureHeight, s16 rectLeft,
    ...
    { GraphicsContext* __gfxCtx = play->state.gfxCtx; (void)__gfxCtx;
...
// Inventory item-level readout: for the item the cursor is highlighting in the pause menu (pauseCtx->namedItem),
// draws its level, current EXP, and EXP-to-next near the item-name panel. Item & equipment pages only.
void Leveled_KaleidoItem_Level(PlayState* play) {
    if (!CVarGetInteger("gLeveled.Master", 1) || !CVarGetInteger("gLeveled.HUD.InventoryItemLevel", 1)) {
        return;
    }
    PauseContext* pauseCtx = &play->pauseCtx;
    // Only while actively browsing items (state 6), with the cursor on an item slot (not a page arrow).
    if (pauseCtx->state != 6 || pauseCtx->cursorSpecialPos != 0) {
        return;
    }
    if (pauseCtx->pageIndex != PAUSE_ITEM && pauseCtx->pageIndex != PAUSE_EQUIP) {
        return;
    }
    // Use the cursor's item (set whenever the cursor is on an item) rather than namedItem, which clears
    // when the name display switches to the "press to equip" prompt.
    s32 item = Leveled_ItemIdToLeveledItem(pauseCtx->cursorItem[pauseCtx->pageIndex]);
    if (item < 0) {
        return;
    }

    u8 level = Leveled_GetItemLevel(item);
    u8 cap = Leveled_GetItemLevelCap(item);
    u32 curExp = Leveled_GetItemExp(item);
    u32 toNext = (level < cap) ? (GetCumulativeExp(level) - curExp) : 0;

    u16 x = 74;
    u16 y = 204; // below the item-name panel
    // "Lv" + level
    Leveled_DrawTexI4(play, dgMsgChar4CLatinCapitalLetterLTex, 16, 16, x, y, 10, 11, 255, 255, 255);
    Leveled_DrawTexI4(play, dgMsgChar76LatinSmallLetterVTex, 16, 16, x + 4, y, 10, 11, 255, 255, 255);
    Leveled_BigValueNumberDraw(play, x + 10, y - 4, level, 255, 241, 79, 255);
    // Current EXP
    y += 10;
    Leveled_DrawTex32(play, dgItemIconGoronsBraceletTex, 32, 32, x - 1, y, 12, 14);
    Leveled_ValueNumberDraw(play, x + 7, y, curExp, 255, 255, 255);
    // EXP to next ("NEXT" + remaining); nothing shown at max level
    y += 8;
    if (level < cap) {
        Leveled_DrawTex4b(play, dgNextDoActionENGTex, 48, 16, x - 4, y, 68, 15);
        Leveled_ValueNumberDraw(play, x + 22, y, toNext, 120, 255, 0);
    }
}