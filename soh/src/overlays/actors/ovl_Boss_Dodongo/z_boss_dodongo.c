...
void BossDodongo_Init(Actor* thisx, PlayState* play) {
    ...
    for (s32 j = 0; j < this->collider.count; j++)
        this->collider.elements[j].info.toucher.damage <<= 1; // Double fire damage (to 1 heart)
        ...
        if (this->collider.elements->info.bumperFlags & 2) {
            ...
            if ((this->actionFunc == BossDodongo_Vulnerable) || (this->actionFunc == BossDodongo_LayDown)) {
                ...
                swordDamage = Leveled_DamageModify(&this->actor, this->collider.base.actor,
                                                   swordDamage * Leveled_GetHealthAttackMultiplier());
